function track!(params)
    ...
    params.course[306, 1] += 0.5  ## Small correction to iron out wrinkle
    params.course[306, 2] += 0.137

    params.course[307:end, 1] .+= 1
    params.course[307:end, 2] .+= 0.2727
    ...
    for i = 1:3
        params.course =
            [params.course; (2 * params.course[end, :] - params.course[end-1, :])']
    end
    ...
    # find s data
    params.course[1, 3] = 0
    for i in 2:size(params.course, 1)
        params.course[i, 3] =
            params.course[i-1, 3] + norm(params.course[i, 1:2] - params.course[i-1, 1:2])
    end

    params.course = [params.course zeros(size(params.course, 1), 5)]

    # use arctan to find heading angle of each segment
    params.course[1:end-1, 5] = atan.(diff(params.course[:, 2]), diff(params.course[:, 1]))

    # use the change in heading to find the curvature (dtheta/ds)
    params.course[1, 4] = 0
    params.course[2:end, 4] = diff(params.course[:, 5]) / 5 #  Divide by 5, as ds=5

    # look for and remove any abrupt change when we hit -pi -> pi
    for i in findall(params.course[:, 4] .> 1)
        params.course[i:end, 5] .-= 2pi
    end
    for i in findall(params.course[:, 4] .< -1)
        params.course[i:end, 5] .+= 2pi
    end

    # find the curvature from the change in angle, corrected for gaps
