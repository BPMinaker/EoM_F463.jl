...
ct = cos(theta)
st = sin(theta)
rot_mtx = [ct st; -st ct]  ## Build rotation matrix
...
while d_next <= d_last  ## If the next point is closer than the last, keep looking forward
	d_last = d_next
	nidx += 1
...
for i in 1:ni  ## Add points to index
	push!(indc,pta)
	pta = pta + 1
end
if indc[1] == 0
